...
var notFull = process.argv.slice(-1)[0] === "no";
...
httpsGet("https://geo.datav.aliyun.com/areas_v2/bound/all.json", function (err, data) {
  ...
  var geoJsonUrls = list.map(function (item) {
    if (item.level !== "district")
      return {
        item: item,
        url: "https://geo.datav.aliyun.com/areas_v2/bound/" + item.adcode + (notFull ? "" : "_full") + ".json",
      };
    return { item: item };
  });
  ...
  var areaUrls = list.map(function (item) {
    ...
    if (index < 3)
      return {
        item: item,
        url: "https://geo.datav.aliyun.com/areas/csv/" + item.adcode + "_" + level[index + 1] + ".json",
      };
    return { item: item };
  });
